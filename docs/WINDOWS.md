...
|------|-------------|-----------------|
| **WSL2 + Ubuntu** (full) | Complete workstation: chezmoi, toolchain, AI agents, nan-* scripts | Everyone who can install WSL2 |
| **Git Bash only** (basic) | nan-* scripts in Git Bash, AI skills/agents installed | Quick start, no WSL2 |
...
If you cannot install WSL2, you can get the `nan-*` scripts running in Git Bash.
...
This installs:
- All `nan-*` scripts to `%USERPROFILE%\.local\bin\`
...
> **Note:** The full toolchain (chezmoi, pacman/apt packages, Python/Node baseline) is NOT installed in Git Bash mode. Only the nan-* helper scripts and AI assets are available.
...
- [CLI_HELPERS.md](CLI_HELPERS.md) — nan-* script reference
