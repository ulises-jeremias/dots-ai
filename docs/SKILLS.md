...
│   workflow-sunstone-credit-data-project/
│   dots-ai-dev-companion/
│   sunstone-dev-companion/
│   ui-ux-pro-max/
│   │   SKILL.md
│   │   skill.json
│   │   scripts/
...
For **client engagement** or **project-specific** AI workflows, use the naming and **workflow** rules in [CLIENT_AI_PLAYBOOKS.md](CLIENT_AI_PLAYBOOKS.md) (Generic vs Sunstone—do not mix on the same task). Register new skills in `skills-registry.yaml`; set **`enabled: false`** when a skill should stay off by default for most engineers.
