...
- For **`dots-*` flags**, run `nan-<tool> --help`; do not cache outdated help text in answers.
...
Route specialized work using **`skill-catalog.yaml`**: **dots-ai-dev-companion** / **sunstone-dev-companion** (companion framing, L2/L3); **workflow-generic-project** and **workflow-sunstone-credit-data-project** (WHAT); **github-cli-workflow** / **gitlab-cli-workflow** (draft PR/MR); **dbt-validation** / **snowflake-validation** (data checks); **clickup-cli**; external **jira-*** pack; external **confluence-*** pack; **workstation-triage** (health); **slack-cli**; **ui-ux-pro-max** (UI depth).

For AI agent assistance, invoke agents using **@mention** in your message (NOT the Task tool): **@planner** (feature planning), **@code-reviewer** (code review), **@security-reviewer** (security), **@tdd-guide** (TDD workflow), **@reference-lookup** (dots-ai examples from awesome-nan), or **@sunstone-delivery** (Sunstone workflow). Agents are defined in `~/.config/opencode/agents/` — they are NOT skills and must NOT be loaded via the skill tool.
