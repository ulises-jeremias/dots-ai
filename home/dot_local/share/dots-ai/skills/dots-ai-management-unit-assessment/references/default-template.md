# Management Unit Assessment

## Review

- Destination:
- Human reviewer:
- Assessment status: Draft / Reviewed / Approved

## Unit Summary

- Management scope:
- Assessment period:
- Systems of record:
- Methodology / workflow expected:
- Overall management health:
- Key strengths:
- Key risks or gaps:
- General confidence level:

## Evidence Summary

| Evidence area | Source link or location | Freshness | Confidence | Notes |
| --- | --- | --- | --- | --- |
| Board / backlog |  |  |  |  |
| Planning / estimation |  |  |  |  |
| Delivery reports |  |  |  |  |
| Risks / issues / OIRs |  |  |  |  |
| Retrospectives / improvements |  |  |  |  |
| Decisions / agreements |  |  |  |  |
| Meeting notes / interviews |  |  |  |  |
| Stakeholder feedback |  |  |  |  |
| Team satisfaction signals |  |  |  |  |

## Scorecard

### Governance and Structure

| Indicator | Score | Confidence | Evidence | Notes |
| --- | --- | --- | --- | --- |
| Methodology definition |  |  |  |  |
| Workflow definition |  |  |  |  |
| Issue definition |  |  |  |  |
| Roles definition |  |  |  |  |
| Risk management |  |  |  |  |
| Progress tracking |  |  |  |  |
| Continuous improvement initiatives | Qualitative |  |  |  |

### Delivery and Execution

| Indicator | Score | Confidence | Evidence | Notes |
| --- | --- | --- | --- | --- |
| Delivery |  |  |  |  |
| Agile methodology implementation |  |  |  |  |
| Sprint planning effectiveness |  |  |  |  |
| Backlog management |  |  |  |  |
| Retrospective process |  |  |  |  |
| Adaptability to change |  |  |  |  |

### Collaboration and Communication

| Indicator | Score | Confidence | Evidence | Notes |
| --- | --- | --- | --- | --- |
| Business knowledge |  |  |  |  |
| Client or stakeholder satisfaction |  |  |  |  |
| Communication |  |  |  |  |
| Stakeholder engagement |  |  |  |  |
| Daily standup effectiveness |  |  |  |  |
| Team satisfaction |  |  |  |  |

### Culture and Values Alignment

| Indicator | Score | Confidence | Evidence | Validator | Notes |
| --- | --- | --- | --- | --- | --- |
| Raise the bar |  |  |  |  |  |
| Do what I say |  |  |  |  |  |
| Geek out |  |  |  |  |  |

### AI-Native Management Readiness

| Indicator | Score | Confidence | Evidence | Notes |
| --- | --- | --- | --- | --- |
| Knowledge structure and documentation density |  |  |  |  |
| Decision traceability and governance explicitness |  |  |  |  |
| Workflow determinism and operational clarity |  |  |  |  |
| Feedback loops and continuous learning signals |  |  |  |  |
| Human signal and cultural transparency |  |  |  |  |

## Findings

### Strengths

- Finding:
  - Evidence:
  - Confidence:

### Risks and Gaps

- Finding:
  - Evidence:
  - Impact:
  - Confidence:

### Missing Evidence

- Evidence needed:
  - Indicator impacted:
  - Suggested source or validator:

## Recommendations

| Priority | Recommendation | Linked finding | Suggested follow-up |
| --- | --- | --- | --- |
|  |  |  |  |

## Assumptions and Limitations

- Assumption:
- Limitation:

## Validation

- Subjective scores validated by:
- Scores needing follow-up:
- Consensus notes:
